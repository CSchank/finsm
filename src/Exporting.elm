...
generateTikz time machine =
    let
        ...
        states = 
            indtBy 4
                <| List.map oneState <| Dict.toList machine.statePositions
        ...
        hashCode = String.dropRight 56 << sha256 << String.append (String.fromInt time)

        oneState (sId, (x,y)) =
            let
                (tx,ty) = (String.fromFloat <| x/scale, String.fromFloat <| y/scale)
            in
            String.concat ["\\node[state,thick] at (",tx,",",ty,") (",hashCode <| stateName sId,") {$",stateName sId,"$};"]

        transitions = 
            indtBy 4
                <| List.map oneTransition <| Dict.toList machine.stateTransitions

        oneTransition ((s0,tId,s1), (x1,y1)) =
            let
                transitionName = case Dict.get tId machine.transitionNames of
                                    Just n -> renderSet2String n
                                    _ -> ""
                (x0,y0) = statePos s0
                (x2,y2) = statePos s1
                ( mx, my ) =
                ...
                position =
                    if 0 <= theta && theta <= pi / 2 then
                        if y1 > 0 then
                            "above"
                        else
                            "below"
                    else if -pi / 2 <= theta && theta <= 0 then
                        if y1 > 0 then
                            "below"
                        else
                            "above"
...
unlines = String.concat << List.intersperse "\n"


indtBy : Int -> List String -> String
indtBy n = unlines << List.map ((++) (String.repeat n " "))